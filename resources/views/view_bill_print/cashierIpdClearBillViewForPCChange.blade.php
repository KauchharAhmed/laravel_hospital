                                ...
                                <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption font-dark">
                                          ...
                                          ?>
                                          IPD CLEARANCE BILL LIST BETWEEN FROM <?php echo date('d M Y',strtotime($from_date)) ;?> TO <?php echo date('d M Y',strtotime($to_date)) ; ?> TO CHANGR PC
                                            </span>
                                              </div> 
                                             {
                                                ...
                                                <tr>
                                                    ...
                                                    <?php 
                                                     if($value->pc_id == '0'){
                                                      ...
                                                      $pc_amt_query = DB::table('pc_ledger')->where('branch_id',$branch_id)->where('invoice',$value->invoice)->where('year_invoice',$value->year_invoice)->where('daily_invoice_number',$value->daily_invoice)->where('cashbook_id',$value->cashbook_id)->where('invoice_type',2)->where('pc_id',$value->pc_id)->first();
                                                    ...
                                                    <td><?php echo $ipd_admission_tr_transaction->payment_amount ;  ?></td>  
                                                      <td><a target="_blank" href="{{URL::to('ipdPCChange/'.$value->invoice.'/'.$value->year_invoice.'/'.$value->daily_invoice.'/'.$value->cashbook_id.'/'.$value->ipd_admission_id)}}"><button type="button" class="btn btn-info btn-sm">CHANGE PC</button></a></td> 
                                                </tr>
                                                <?php } ?>
                                            </tbody>
                                        </table>
