                                ...
                                <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption font-dark">
                                          ...
                                          ?>
                                          IPD PATHOLOGY BILL REPORT BETWEEN FROM <?php echo date('d M Y',strtotime($from_date)) ;?> TO <?php echo date('d M Y',strtotime($to_date)) ; ?>
                                            </span>
                                              </div> 
                                             {!! Form::open(['url' =>'printCashierIpdPathologyBillReport','method' => 'post','role' => 'form','class'=>'form-horizontal','target'=>'_blank']) !!}
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    echo "IPDA- ".$ipd_admit_query->invoice ;
                                                     ?></td>
                                                    <td><?php echo "IPDP- ".$value->invoice ; ?></td>
                                                    <td><?php echo $value->patient_number ; ?></td>
                                                    <td><?php echo $value->patient_name ; ?></td>
                                                    <?php
                                                    // get tr info
                                                     $ipd_admission_tr_transaction = DB::table('tbl_ipd_ledger')->where('branch_id',$branch_id)->where('ipd_admission_id',$value->ipd_admission_id)->where('service_invoice',$value->invoice)->where('service_id',$value->id)->where('service_type',2)->first(); 
                                                    ?>
                                                    ...
                                                    <?php if($setting_status == '2'){?>
                                                     <td>
                                                    <a href="{{URL::to('cashierDeleteIPDPathology/'.$value->id.'/'.$value->ipd_admission_id.'/'.$value->invoice.'/'.$value->year_invoice.'/'.$value->daily_invoice.'/'.$value->cashbook_id)}}"><button type="button" class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure You Want To Delete It ?')">DELETE</button></a>
                                                    </td> 
                                                    <?php } ?> 
                                                </tr>
                                                <?php } ?>
