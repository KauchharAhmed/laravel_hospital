                                ...
                                <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption font-dark">
                                          ...
                                          ?>
                                          OT SERVICE BILL REPORT BETWEEN FROM <?php echo date('d M Y',strtotime($from_date)) ;?> TO <?php echo date('d M Y',strtotime($to_date)) ; ?>
                                          
                                            </span>
                                              </div> 
                                             {!! Form::open(['url' =>'printCashierOTServiceBillReport','method' => 'post','role' => 'form','class'=>'form-horizontal','target'=>'_blank']) !!}
                                        ...
                                        <div class="table-responsive">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($result as $value) { 
                                                  // get ot booking info
                                                  $ot_book_query = DB::table('tbl_ot_booking')->where('id',$value->ot_booking_id)->first();
                                                 ?>
                                                <tr>
                                                    <td><?php echo $i++ ;?></td>
                                                    ...
                                                    <td><?php echo "OTS- ".$value->invoice ; ?></td>
                                                    ...
                                                    <?php
                                                    // get tr info
                                                     $ot_booking_tr_transaction = DB::table('tbl_ot_ledger')->where('branch_id',$branch_id)->where('ot_booking_id',$value->ot_booking_id)->where('service_invoice',$value->invoice)->where('service_id',$value->id)->where('service_type',3)->first(); 
                                                    ?>
                                                    <td><?php echo $ot_booking_tr_transaction->payable_amount ;  ?></td> 
                                                    <td><?php echo $ot_booking_tr_transaction->discount ;  ?></td>  
                                                    <?php if($setting_status == '2'){?>
                                                    <td><a href="{{URL::to('cashierDeleteOTService/'.$value->id.'/'.$value->ot_booking_id.'/'.$value->invoice.'/'.$value->year_invoice.'/'.$value->daily_invoice)}}"><button type="button" class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure You Want To Delete It ?')">DELETE</button></a></td>  
                                                    <?php } ?>
                                                </tr>
                                                <?php } ?>
                                                <tr>
