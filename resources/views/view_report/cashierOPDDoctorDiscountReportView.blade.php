                                ...
                                <div class="portlet light bordered">
                                    <div class="portlet-title">
                                        <div class="caption font-dark">
                                          ...
                                          ?>
                                          OPD DOCTOR DISCOUNT REPORT BETWEEN FROM <?php echo date('d M Y',strtotime($from_date)) ;?> TO <?php echo date('d M Y',strtotime($to_date)) ; ?>
                                            </span>
                                              </div> 
                                             {!! Form::open(['url' =>'printCashierOPDDoctorDiscountReport','method' => 'post','role' => 'form','class'=>'form-horizontal','target'=>'_blank']) !!}
                                        ...
                                        <div class="table-responsive">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($pathology_tr_transaction as $value) { 
                                                  // pathology bill info
                                                  $pathology_bill_info = DB::table('opd_bill')->where('branch_id',$branch_id)->where('invoice',$value->invoice_number)->where('year_invoice',$value->year_invoice_number)->where('daily_invoice',$value->daily_invoice_number)->first();
                                                  // get patient info
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo "OPD- ".$pathology_bill_info->invoice ; ?></td>
                                                    ...
                                                    <?php if($setting_status == '2'){?>
                                                    <td><a target="_blank" href="{{URL::to('cashierDeleteOPDDoctorDiscount/'.$value->id.'/'.$value->invoice_number.'/'.$value->year_invoice_number.'/'.$value->daily_invoice_number.'/'.$value->cashbook_id.'/'.$value->invoice_tr_id.'/'.'2')}}"><button type="button" class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure You Want To Delete It ?')">DELETE</button></a></td> 
                                                    <?php } ?> 
                                                </tr>
                                                <?php } ?>
                                                <tr>
